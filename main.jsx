...
function fmtDate(d) {
  if (!d) return "—";
  // Se for formato YYYY-MM-DD (data programada), adiciona meio-dia para evitar bug de fuso horário
...
function convertUrl(url) {
  if (!url) return url;
  // Google Drive: converte /file/d/ID/view para embed
...
function isVid(url, type) {
  ...
  if (url.indexOf("drive.google.com") >= 0) return true;
  return false;
}
...
function MediaPreview({ url, mediaType, height, width }) {
  ...
  if (vid) {
    ...
    // Google Drive
    if (url.indexOf("drive.google.com") >= 0) return <div style={{ width: w, height: h, background: "#000", flexShrink: 0 }}><iframe src={converted} style={{ width: "100%", height: "100%", border: "none" }} allowFullScreen title="drive" /></div>;
    // Vídeo direto
  ...
  return (
    <div>
      <div style={aspectStyle}>
        {isVid(list[idx], "")
          ? (list[idx].indexOf("drive.google.com") >= 0
              ? <iframe src={convertUrl(list[idx])} style={{ width: "100%", height: "100%", border: "none" }} allowFullScreen title="drive" />
              : list[idx].match(/(?:v=|youtu\.be\/)([^&\s]+)/)
                ? <iframe src={"https://www.youtube.com/embed/" + list[idx].match(/(?:v=|youtu\.be\/)([^&\s]+)/)[1]} style={{ width: "100%", height: "100%", border: "none" }} allowFullScreen title="yt" />
...
function PostForm({ initial, onSave, onClose, clientId, area }) {
  ...
  async function uploadToStorage(file) {
    var ext = file.name.split(".").pop() || (file.type.startsWith("video/") ? "mp4" : "jpg");
    var filename = uid() + "." + ext;
    var path = "posts/" + filename;
    var res = await fetch(SUPABASE_URL + "/storage/v1/object/midias/" + path, {
      method: "POST",
      headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY, "Content-Type": file.type, "x-upsert": "true" },
      body: file
    });
    if (!res.ok) return null;
    return SUPABASE_URL + "/storage/v1/object/public/midias/" + path;
  }
  ...
  function handleFiles(e) {
    ...
    files.forEach(function(file, i) {
      uploadToStorage(file).then(function(url) {
        results[i] = url || "";
        loaded++;
        if (loaded === files.length) {
          setMedias(function(m) { return m.concat(results.filter(Boolean)); });
          setUploading(false);
        }
      }).catch(function() {
        loaded++;
    ...
    <div className="pcard" style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: 12, overflow: "hidden", marginBottom: 10 }}>
      <div style={{ display: "flex" }}>
        <div style={{ width: isStory ? 66 : 110, minHeight: 110, flexShrink: 0, position: "relative" }}>
          <MediaPreview url={firstUrl} mediaType={firstUrl && isVid(firstUrl, "") ? "Vídeo" : "Imagem"} height={110} width={isStory ? 66 : 110} />
        ...
        <div style={{ borderTop: "1px solid " + BORDER }}>
          {mediaUrls.length > 0 && (
            <div style={{ borderBottom: "1px solid " + BORDER }}>
              <CarouselViewer urls={mediaUrls} mediaType={mediaUrls[0] && isVid(mediaUrls[0], "") ? "Vídeo" : "Imagem"} isStory={isStory} />
            </div>
          )}
          <div style={{ padding: "20px 20px 0" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ background: W05, border: "1px solid " + BORDER, borderRadius: 8, padding: "10px 14px" }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: MUTED, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 4 }}>ENVIADO PARA APROVAÇÃO</div>
                    <div style={{ fontSize: 13, color: W60 }}>{fmtDate(post.created_at)}</div>
                  </div>
                  <div style={{ background: post.scheduled_at ? "rgba(127,255,127,0.06)" : W05, border: "1px solid " + (post.scheduled_at ? "rgba(127,255,127,0.3)" : BORDER), borderRadius: 8, padding: "10px 14px" }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: post.scheduled_at ? GREEN : MUTED, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 4 }}>📅 DATA DE PUBLICAÇÃO</div>
                    {post.scheduled_at
                      ? <div style={{ fontSize: 15, fontWeight: 800, color: GREEN }}>{fmtDate(post.scheduled_at)}</div>
                      : <div style={{ fontSize: 13, color: MUTED, fontStyle: "italic" }}>Não definida</div>
                    }
                  </div>
                </div>
              </div>
...
function Panel({ user, clientId, isCreator, onLogout }) {
  ...
  function doLoad(attempt, onDone) {
    sbSelect("posts", "client_id=eq." + clientId + "&order=scheduled_at.desc.nullslast,created_at.desc")
      .then(function(rows) {
        if ((!rows || rows.length === 0) && attempt < 4) {
          setTimeout(function() { doLoad(attempt + 1, onDone); }, 1500);
          return;
        }
        onDone(rows || []);
      })
      .catch(function() {
        if (attempt < 4) { setTimeout(function() { doLoad(attempt + 1, onDone); }, 1500); }
        else { onDone([]); }
      });
  }
  ...
  function loadPosts() {
    setLoading(true);
    doLoad(1, function(rows) {
      var now = Date.now();
      var twoDays   = 2 * 24 * 60 * 60 * 1000;
      var twoMonths = 60 * 24 * 60 * 60 * 1000;

      var toAutoApprove = rows.filter(function(p) {
        if (p.status !== "pending" || !p.scheduled_at) return false;
        return (now - new Date(p.scheduled_at + "T12:00:00").getTime()) >= twoDays;
      });
      var toDelete = rows.filter(function(p) {
        if (!p.scheduled_at) return false;
        return (now - new Date(p.scheduled_at + "T12:00:00").getTime()) >= twoMonths;
      });

      var actions = [];
      toAutoApprove.forEach(function(p) { actions.push(sbUpdate("posts", p.id, { status: "approved", revision_note: "" })); });
      toDelete.forEach(function(p) { actions.push(sbDelete("posts", p.id)); });

      function sortAndSet(list) {
        list.sort(function(a, b) {
          var da = a.scheduled_at ? new Date(a.scheduled_at + "T12:00:00").getTime() : 0;
          var db = b.scheduled_at ? new Date(b.scheduled_at + "T12:00:00").getTime() : 0;
          return db - da;
        });
  ...
  // Retry automático se vier vazio (pode ser lentidão do banco)
  useEffect(function() {
    if (!loading && posts.length === 0) {
      var timer = setTimeout(function() { loadPosts(); }, 2000);
  ...
  function updatePost(form) {
    ...
    sbUpdate("posts", editPost.id, updates).then(function() { loadPosts(); setSaving(false); setEditPost(null); });
  }
      ...
      <div style={{ padding: "24px 22px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        {tab === "calendar" ? (
          ...
          <div>
            ...
            {!isCreator && (
              <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid " + BORDER, borderRadius: 8, padding: "10px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 15 }}>ℹ️</span>
                <span style={{ fontSize: 11, color: MUTED, lineHeight: 1.5 }}>
                  Os posts são removidos automaticamente <strong style={{ color: WHITE }}>60 dias após a data de publicação</strong>. Salve o conteúdo antes desse prazo se necessário.
                </span>
              </div>
            )}
            <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 5 }}>
                {["pending","approved","revision","all"].map(function(f) {
            ...
            {!loading && filtered.length === 0 && (
              <div style={{ textAlign: "center", padding: 60, border: "1px dashed " + BORDER, borderRadius: 12, background: CARD }}>
                ...
                <div style={{ fontWeight: 600, color: WHITE, fontSize: 12, letterSpacing: "1px" }}>
                  {posts.length === 0 ? (isCreator ? "NENHUM POST PARA ESTE CLIENTE" : "NENHUM CONTEÚDO AINDA") : "NENHUM RESULTADO"}
                </div>
                {posts.length === 0 && !isCreator && <div style={{ fontSize: 11, color: MUTED, marginTop: 6 }}>Aguarde a BW Produções enviar seu conteúdo</div>}
                {posts.length === 0 && (
                  <button className="btn-ghost" onClick={function() { setLoading(true); loadPosts(); }}
                    style={{ marginTop: 16, padding: "8px 20px", background: "transparent", border: "1px solid " + BORDER, borderRadius: 8, color: MUTED, fontSize: 11, fontWeight: 700, cursor: "pointer", letterSpacing: "1px" }}>
                    ↻ RECARREGAR
                  </button>
                )}
              </div>
