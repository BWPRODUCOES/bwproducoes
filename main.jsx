    ...
    sbSelect("posts", "client_id=eq." + clientId + "&order=scheduled_at.desc.nullslast,created_at.desc")
      .then(function(rows) {
        ...
        var toAutoApprove = rows.filter(function(p) {
          ...
          var scheduledDate = new Date(p.scheduled_at + "T12:00:00").getTime();
          return (now - scheduledDate) >= twoDays;
        });
        if (toAutoApprove.length > 0) {
          Promise.all(toAutoApprove.map(function(p) {
            return sbUpdate("posts", p.id, { status: "approved", revision_note: "" });
          })).then(function() {
            sbSelect("posts", "client_id=eq." + clientId + "&order=scheduled_at.desc.nullslast,created_at.desc")
