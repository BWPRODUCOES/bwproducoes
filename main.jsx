...
// ── CARROSSEL VIEWER ──────────────────────────────────────────────────────────
function CarouselViewer({ urls, mediaType, height, thumbSize }) {
  ...
  return (
    <div>
      <div style={{ position: "relative" }}>
        <MediaPreview url={list[idx]} mediaType={mediaType} height={h} width="100%" />
        {list.length > 1 && (
          <div style={{ position: "absolute", top: "50%", left: 0, right: 0, transform: "translateY(-50%)", display: "flex", justifyContent: "space-between", padding: "0 10px", pointerEvents: "none" }}>
            <button onClick={function(e) { e.stopPropagation(); setIdx(function(i) { return i === 0 ? list.length - 1 : i - 1; }); }}
              style={{ pointerEvents: "all", width: 32, height: 32, borderRadius: "50%", background: "rgba(0,0,0,.6)", border: "none", color: WHITE, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
        ...
        {list.length > 1 && (
          <div style={{ position: "absolute", bottom: 10, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 5 }}>
            {list.map(function(_, i) {
              return <div key={i} onClick={function() { setIdx(i); }} style={{ width: i === idx ? 18 : 6, height: 6, borderRadius: 3, background: i === idx ? WHITE : "rgba(255,255,255,0.4)", cursor: "pointer", transition: "all .2s" }} />;
            })}
          </div>
        )}
      </div>
          ...
          {list.map(function(u, i) {
            return (
              <div key={i} onClick={function() { setIdx(i); }}
                style={{ width: ts, height: ts, flexShrink: 0, borderRadius: 6, overflow: "hidden", cursor: "pointer", border: i === idx ? "2px solid " + WHITE : "2px solid transparent", opacity: i === idx ? 1 : 0.5, transition: "all .2s" }}>
                {isVid(u, mediaType)
        ...
        <div style={{ borderTop: "1px solid " + BORDER }}>
          {mediaUrls.length > 0 && (
            <div style={{ borderBottom: "1px solid " + BORDER }}>
              <CarouselViewer urls={mediaUrls} mediaType={post.area === "Story" ? "Vídeo" : "Imagem"} height={isStory ? 480 : 340} />
            </div>
          )}
    ...
    <div style={{ minHeight: "100vh", background: BG }}>
      ...
      <Modal open={!!editPost} onClose={function() { setEditPost(null); }}>
        {editPost && <PostForm initial={{
          ...
          platform: editPost.platform, area: editPost.area, scheduledAt: editPost.scheduled_at,
