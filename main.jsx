  ...
  function loadPosts() {
    sbSelect("posts", "client_id=eq." + clientId + "&order=scheduled_at.desc.nullslast,created_at.desc")
      .then(function(rows) {
        ...
        // Posts para aprovar automaticamente (2 dias após publicação)
        var toAutoApprove = rows.filter(function(p) {
          if (p.status !== "pending") return false;
          if (!p.scheduled_at) return false;
          var d = new Date(p.scheduled_at + "T12:00:00").getTime();
          return (now - d) >= twoDays;
        });

        // Posts para excluir automaticamente (2 meses após publicação)
        var toDelete = rows.filter(function(p) {
          if (!p.scheduled_at) return false;
          var d = new Date(p.scheduled_at + "T12:00:00").getTime();
          return (now - d) >= twoMonths;
        });

        var actions = [];
        toAutoApprove.forEach(function(p) {
          actions.push(sbUpdate("posts", p.id, { status: "approved", revision_note: "" }));
        });
        toDelete.forEach(function(p) {
          actions.push(sbDelete("posts", p.id));
        });
          ...
          Promise.all(actions).then(function() {
            sbSelect("posts", "client_id=eq." + clientId + "&order=scheduled_at.desc.nullslast,created_at.desc")
              .then(function(updated) {
                ...
                setPosts(updated); setLoading(false);
              });
          });
        } else {
