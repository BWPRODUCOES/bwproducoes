...
function PostForm({ initial, onSave, onClose, clientId, area }) {
  var [form, setForm] = useState(initial || { title: "", caption: "", platform: "Instagram", area: area || "Feed", scheduledAt: "", mediaUrl: "", hashtags: "", notes: "" });
  var [uploadTab, setUploadTab] = useState("url");
  ...
  function handleFile(e) {
    ...
    reader.onload = function(ev) { setForm(function(f) { var n = Object.assign({}, f); n.mediaUrl = ev.target.result; return n; }); setUploading(false); };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div>
        <label style={lbl}>Mídia — imagem ou vídeo</label>
        <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
          {["url","upload"].map(function(t) {
            return <button key={t} onClick={function() { setUploadTab(t); }}
              style={{ padding: "7px 16px", borderRadius: 6, border: uploadTab === t ? "1px solid " + WHITE : "1px solid " + BORDER, background: uploadTab === t ? WHITE : "transparent", color: uploadTab === t ? BG : MUTED, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
              {t === "url" ? "URL" : "UPLOAD"}</button>;
          })}
        </div>
        {uploadTab === "url"
          ? <input style={inp} value={form.mediaUrl} onChange={sf("mediaUrl")} placeholder="https://… (imagem, vídeo, YouTube, Vimeo)" />
          : <div>
              <input ref={fileRef} type="file" accept="image/*,video/*" onChange={handleFile} style={{ display: "none" }} />
              <button className="btn-ghost" onClick={function() { if (fileRef.current) fileRef.current.click(); }}
                style={{ width: "100%", background: W05, border: "1px dashed " + BORDER2, borderRadius: 8, padding: 24, cursor: "pointer", color: MUTED, fontSize: 13 }}>
                {uploading ? "Carregando…" : (form.mediaUrl && form.mediaUrl.startsWith("data:")) ? "✓ Carregado — clique para trocar" : "Selecionar imagem ou vídeo"}</button>
            </div>}
        {form.mediaUrl && <div style={{ marginTop: 10, borderRadius: 8, overflow: "hidden", border: "1px solid " + BORDER }}>
          <MediaPreview url={form.mediaUrl} mediaType={form.area === "Story" ? "Vídeo" : "Imagem"} height={180} width="100%" /></div>}
      </div>
    ...
    <div className="pcard" style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: 12, overflow: "hidden", marginBottom: 10 }}>
      <div style={{ display: "flex" }}>
        <div style={{ width: isStory ? 66 : 110, minHeight: 110, flexShrink: 0, position: "relative" }}>
          <MediaPreview url={post.media_url} mediaType={post.area === "Story" ? "Vídeo" : "Imagem"} height={110} width={isStory ? 66 : 110} />
        ...
        <div style={{ flex: 1, padding: "13px 15px 11px", display: "flex", flexDirection: "column", gap: 7, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
            ...
            {post.scheduled_at && <span style={{ fontSize: 9, color: MUTED }}>📅 {fmtDate(post.scheduled_at)}</span>}
          </div>
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid " + BORDER }}>
          {post.media_url && <div style={{ borderBottom: "1px solid " + BORDER }}>
            <MediaPreview url={post.media_url} mediaType={post.area === "Story" ? "Vídeo" : "Imagem"} height={isStory ? 480 : 340} width="100%" /></div>}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ fontSize: 11, color: MUTED2 }}>
                  <div>Criado: {fmtDate(post.created_at)}</div>
                  {post.scheduled_at && <div style={{ marginTop: 4 }}>Agendado: {fmtDate(post.scheduled_at)}</div>}
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG }}>
      ...
      <Modal open={!!editPost} onClose={function() { setEditPost(null); }}>
        {editPost && <PostForm initial={{ title: editPost.title, caption: editPost.caption, hashtags: editPost.hashtags, platform: editPost.platform, area: editPost.area, scheduledAt: editPost.scheduled_at, mediaUrl: editPost.media_url, notes: editPost.notes }} onSave={updatePost} onClose={function() { setEditPost(null); }} clientId={clientId} />}
      </Modal>
    </div>
  );
}
